...
FILENAME = "sqr_array_3.c"

requirements = {
			"int_array": {
				"text": "int-Array der Größe 1000",
				"checked": False
			}
		}
...
def check_exercise(filename):
	...
	if not requirements["int_array"]["checked"]:
		...
		requirements["int_array"]["checked"] = True
		try:
			int_array = next(x for x in main_ast.body.block_items if type(x) == c_ast.Decl and type(x.type) == c_ast.ArrayDecl and x.type.type.type.names == ["int"] and x.type.dim.value == "1000")
		except StopIteration:
			requirements["int_array"]["checked"] = False
